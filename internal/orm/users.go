...
type userR struct {
	UserEmails   UserEmailSlice   `boil:"UserEmails" json:"UserEmails" toml:"UserEmails" yaml:"UserEmails"`
...
var (
	userAllColumns            = []string{"id", "created_at", "updated_at", "removed_at", "password", "is_active", "is_banned"}
	userColumnsWithoutDefault = []string{"removed_at", "is_active", "is_banned"}
	userColumnsWithDefault    = []string{"id", "created_at", "updated_at", "password"}
...
// UserEmails retrieves all the user_email's UserEmails with an executor.
func (o *User) UserEmails(mods ...qm.QueryMod) userEmailQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"user_emails\".\"user_id\"=?", o.ID),
	)

	query := UserEmails(queryMods...)
	queries.SetFrom(query.Query, "\"user_emails\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"user_emails\".*"})
	}
...
func (o *User) UserProfiles(mods ...qm.QueryMod) userProfileQuery {
	...
	queries.SetFrom(query.Query, "\"user_profiles\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"user_profiles\".*"})
	}
...
// LoadUserEmails allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserEmails(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`user_emails`),
		qm.WhereIn(`user_emails.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_emails")
	}

	var resultSlice []*UserEmail
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_emails")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_emails")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_emails")
	}
	...
	if singular {
		object.R.UserEmails = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.UserID) {
				local.R.UserEmails = append(local.R.UserEmails, foreign)
				if foreign.R == nil {
					foreign.R = &userEmailR{}
				}
...
// AddUserEmails adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserEmails.
// Sets related.R.User appropriately.
func (o *User) AddUserEmails(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserEmail) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.UserEmails = append(o.R.UserEmails, related...)
	}
...
// SetUserEmails removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's UserEmails accordingly.
// Replaces o.R.UserEmails with related.
// Sets related.R.User's UserEmails accordingly.
func (o *User) SetUserEmails(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserEmail) error {
	query := "update \"user_emails\" set \"user_id\" = null where \"user_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.UserEmails {
	...
	return o.AddUserEmails(ctx, exec, insert, related...)
}

// RemoveUserEmails relationships from objects passed in.
// Removes related items from R.UserEmails (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemoveUserEmails(ctx context.Context, exec boil.ContextExecutor, related ...*UserEmail) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.UserEmails {
			if rel != ri {
				continue
			}

			ln := len(o.R.UserEmails)
			if ln > 1 && i < ln-1 {
				o.R.UserEmails[i] = o.R.UserEmails[ln-1]
			}
			o.R.UserEmails = o.R.UserEmails[:ln-1]
			break
		}
	}
...
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's UserProfiles accordingly.
// Replaces o.R.UserProfiles with related.
// Sets related.R.User's UserProfiles accordingly.
func (o *User) SetUserProfiles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserProfile) error {
	query := "update \"user_profiles\" set \"user_id\" = null where \"user_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
...
// Sets related.R.User.
func (o *User) RemoveUserProfiles(ctx context.Context, exec boil.ContextExecutor, related ...*UserProfile) error {
	if len(related) == 0 {
		return nil
	}
...
// SetUserTokens removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's UserTokens accordingly.
// Replaces o.R.UserTokens with related.
// Sets related.R.User's UserTokens accordingly.
func (o *User) SetUserTokens(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserToken) error {
	query := "update \"user_tokens\" set \"user_id\" = null where \"user_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	if o.R != nil {
		for _, rel := range o.R.UserTokens {
			queries.SetScanner(&rel.UserID, nil)
			if rel.R == nil {
				continue
			}
	...
	return o.AddUserTokens(ctx, exec, insert, related...)
}

// RemoveUserTokens relationships from objects passed in.
// Removes related items from R.UserTokens (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemoveUserTokens(ctx context.Context, exec boil.ContextExecutor, related ...*UserToken) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.UserTokens {
			if rel != ri {
				continue
			}

			ln := len(o.R.UserTokens)
			if ln > 1 && i < ln-1 {
				o.R.UserTokens[i] = o.R.UserTokens[ln-1]
			}
			o.R.UserTokens = o.R.UserTokens[:ln-1]
			break
		}
	}
